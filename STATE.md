...
- I3 전 모듈 계층 배정과 선언 기반 정책·상태 발견: `pass`
...
1. `failures/`의 각 사례를 규칙·테스트 흡수 여부와 남은 비자명 정보로 분류한다.
2. G5와 실패 기록 규칙의 기본 행동을 새 문서 생성이 아닌 예방책 흡수로 바꾼다.
3. 실패 문서를 기본 route·context에 추가하지 않고, 삭제 없이 중복 소유를 제거해 I4 게이트를 실행한다.
