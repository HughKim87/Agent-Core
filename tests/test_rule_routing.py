...
class RoutingCompletenessTest(unittest.TestCase):
    ...
    def test_generalization_gate_structure_anchor(self) -> None:
        # Navigation anchor only; independent semantic audit owns meaning.
        self.assertIn("## 2. Core 일반화 적합성 게이트", read(ROOT / "rules/rule-governance.md"))
...
class FixtureStructureTest(unittest.TestCase):
    ...
    def test_referenced_owners_are_routed(self) -> None:
        ...
        for case in self.cases:
            for owner in case["prior_owners"] + case["expected_owners"] + case["forbidden_owners"]:
...
class FailureSafetyScenarioTest(unittest.TestCase):
    """General synthetic stop predicates, not an Agent implementation test."""

    def test_general_scenarios(self) -> None:
        data = json.loads(read(FAILURE_FIXTURE))
        self.assertEqual(data["evidence_level"], "synthetic-policy-model")
        self.assertEqual(routed_owners().count(data["policy_owner"]), 1)
        self.assertNotIn("failure_limit", data)
        self.assertTrue(data["cases"])
        for case in data["cases"]:
            with self.subTest(case=case["id"]):
                eligible = (case["budget_available"] and case["progress"]
                            and case["external_resolved"] and not case["cancelled"]
                            and case["conflict_free"])
                self.assertEqual("eligible" if eligible else "stop", case["expected"])
